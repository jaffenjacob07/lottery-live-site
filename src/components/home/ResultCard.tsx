    ...
    >
      <div
        className={`bg-gradient-to-r from-navy-900 via-accent-red to-navy-900 ${
          featured ? "sm:w-1.5 sm:min-h-full h-2 sm:h-auto w-full" : "h-1.5"
        }`}
      />
      <div className={`flex-1 p-5 sm:p-6 ${featured ? "sm:py-7" : ""}`}>
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <Trophy
            className={`text-navy-200 group-hover:text-accent-red transition-colors shrink-0 ${
              featured ? "h-10 w-10 sm:h-12 sm:w-12" : "h-8 w-8"
            }`}
          />
        </div>
        ...
        <dl className="space-y-2 text-sm mb-5">
          <div className="flex justify-between gap-4">
            <dt className="text-navy-500 shrink-0">Date</dt>
            <dd className="font-medium text-navy-800 text-right">{formatDate(result.date)}</dd>
          </div>
          <div className="flex justify-between items-center gap-4">
            <dt className="text-navy-500 shrink-0">Location</dt>
